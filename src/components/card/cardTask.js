...
const CardTask = ({ data, handleRemoveTask }) => {
  ...
  const [statusTaskTemp, setStatusTaskTemp] = useState(data.status);
  ...
  const handleUpdateTask = () => {
    if (!submiting) { // chưa nhấn create
      ...
      axios.patch(`${BASE_URL}/task/update/${data._id}`, formData, {
        ...
        setStatusTaskTemp(statusTask) // cập nhật biến tạm của status // nếu status hiện tại khác với status trước đó thì ẩn đi
        setSubmiting(false)
